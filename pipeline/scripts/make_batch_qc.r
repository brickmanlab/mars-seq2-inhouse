...
if (length(args)==2){
  ...
  message("Usage: Rscript make_batch_qc.r batch output_dir")
  stop()
}
...
make_amp_batch_qc_fig=function(amp_batch_ID,output_dir="output"){
  ...
  batch_title=paste("batch #",amp_batch_ID," (",paste(batch_details["Owner"]," Exp_ID=",batch_details["Experiment_ID"],sep=""),")",sep="")
  ...
  umi_data=read.table(paste(output_dir,"/QC/umi_stats/",amp_batch_ID,".txt",sep=""),header=T,row.names=1)

  x=read_stats[mask1,]

  layout(matrix(c(1,1,1,1,2:11,12,13,14:17,18,18,19,19,20:23),7,4,byrow=T),heights=c(1,rep(2,6)))
  ...
  mbar=cbind(gene_mapped=x[,"gene_mapped"],gene_filtered=rowSums(x[,c("gene_UMI_err","gene_barcode_err","gene_lonely_offset_err","gene_readpoor_offset_err" )]),mapped_to_nongenic=x[,"mapped_to_nongenic"],spike_mapped=x[,"spike_mapped"],spike_filtered=rowSums(x[,c("spike_UMI_err","spike_barcode_err","spike_lonely_offset_err","spike_readpoor_offset_err")]),low_mapq=x[,"low_mapq"],multimap=x[,"non_unique_mapping"])/tot
  ...
  message("B")
  unmapped_stats=colSums(x[,setdiff(colnames(x),c('well_id','low_mapq','non_unique_mapping','mapped_to_nongenic','spike_UMI_err','spike_barcode_err','spike_lonely_offset_err','spike_readpoor_offset_err','spike_mapped','gene_UMI_err','gene_barcode_err','gene_lonely_offset_err','gene_readpoor_offset_err','gene_mapped'))])
  ...
  m=read.table(paste(output_dir,"/QC/umi_nuc_per_pos/",amp_batch_ID,".txt",sep=""),header=T,row.names=1)
  freq_per_pos=m/rowSums(m)
  matplot(freq_per_pos,pch=c("A","C","G","T"),col=c(3,4,"orange","red"),cex=.8,ylab="Freq.",xlab="position")
  title("UMI nuc/position")
  mtext("D",side=3,adj=-.6,line=2)  
  message("D")

  genetot=rowSums(umi_data[mask1,c("gene_UMI_err","gene_barcode_err","gene_readpoor_offset_err","gene_lonely_offset_err","gene_ok")])
  spiketot=rowSums(umi_data[mask1,c("spike_UMI_err","spike_barcode_err","spike_readpoor_offset_err","spike_lonely_offset_err","spike_ok")])
  
  plot_cumulative(x=umi_data[mask1,"gene_UMI_err"]/genetot,y=umi_data[mask1,"spike_UMI_err"]/spiketot,main="UMI Errors",xlab="Raw UMI fraction",ylab="Fraction of cells",col=gene_spike_col)
  legend("bottomright",legend=c("Genes","Spikes"),lty=1,col=c(colors()[132],"firebrick"),cex=0.6,bg="white")
  mtext("E",side=3,adj=-.6,line=2)
 message("E")
 
  plot_cumulative(x=umi_data[mask1,"gene_barcode_err"]/genetot,y=umi_data[mask1,"spike_barcode_err"]/spiketot,main="Barcode Errors",xlab="Raw UMI fraction",ylab="Fraction of cells",col=gene_spike_col)
   legend("bottomright",legend=c("Genes","Spikes"),lty=1,col=c(colors()[132],"firebrick"),cex=0.6,bg="white")

  mtext("F",side=3,adj=-.6,line=2)
  message("F")
 
  gene_offset_errors=(umi_data[mask1,"gene_lonely_offset_err"]+umi_data[mask1,"gene_readpoor_offset_err"])/genetot
  spike_offset_errors=(umi_data[mask1,"spike_lonely_offset_err"]+umi_data[mask1,"spike_readpoor_offset_err"])/spiketot
 
  if (any(is.na(gene_offset_errors))|any(is.na(spike_offset_errors))){
    plot.new()
  }
  else{
    plot_cumulative(x=gene_offset_errors,y=spike_offset_errors,main="Switchers",xlab="Raw UMI fraction",ylab="Fraction of cells",col=gene_spike_col)
     legend("bottomright",legend=c("Genes","Spikes"),lty=1,col=c(colors()[132],"firebrick"),cex=0.6,bg="white")
  }
 ...
 message("H")
  ...
  plot(nmols_spike,log10(nmols_genes+1),xlab="#spike_umis",ylab="log10(#gene_umis)",panel.first=grid(),col=ifelse(colnames(umitab_batch)%in%mask1,1,2),pch=ifelse(colnames(umitab_batch)%in%mask1,20,4),cex=ifelse(colnames(umitab_batch)%in%mask1,.5,2))
 # text(z[,1],z[,2],labels=z$well_coor,col=ifelse(!z$is_empty,colors()[132],"firebrick"),cex=.6)
  ...
  plot(colSums(umitab_batch[gene_mask,]),100*colSums(singleoffsetab_batch[gene_mask,])/colSums(umitab_batch[gene_mask,]),log="x",pch=ifelse(single_cell_mask,20,4),col=ifelse(single_cell_mask,1,2),ylim=c(0,100),xlab="#UMIs",ylab="%Singleton IVT prod.",panel.first=grid(lty=1),cex=ifelse(single_cell_mask,.5,2))
 ...
 message("M")
  ...
  mtext(side=3,text=paste("Median=",med,paste("Yield=",main_stats[["spike_yield"]],"%"),sep=" ") ,col=2,cex=.7)
 ...
 message("N")
  ...
  noffsets_per_umi=read.table(paste(output_dir,"/QC/noffsets_per_umi_distrib/",amp_batch_ID,".txt",sep=""),header=T,row.names=1)
  barplot2(log10(1+t(noffsets_per_umi[2:20,3])),col="gray",main="#IVT products/UMI",names=1:19,plot.grid=T,border=F,ylab="log10(counts)",space=0)
  ...
  nreads_per_umi=read.table(paste(output_dir,"/QC/nreads_per_umi_distrib/",amp_batch_ID,".txt",sep=""),header=T,row.names=1)
  barplot2(log10(1+t(nreads_per_umi[2:51,3])),col="gray",main="#Reads/UMI",names=1:50,plot.grid=T,border=F,ylab="log10(counts)",space=0)
...
get_stats_per_seq_batch=function(seq_batch){
  ...
  return(round(c(n_cells=n_cells,avg_mapping_stats,avg_n_UMIs=avg_n_UMI,total_n_UMIs=total_n_UMI,total_n_raw_reads=total_n_raw_reads,avg_n_reads_per_UMI=avg_n_reads_per_UMI)))
}
