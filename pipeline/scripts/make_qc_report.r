...
m2=t(sapply(main_stats_list,function(x){return(x[c("gene_umis","gene_umis_neg_control","avg_noffsets_per_umi","avg_noffsets_per_umi_neg_ctrl")])}))


# UMIs vs. QC1
try({
plot(amp_batches$QC1..pooled.cDNA.,m2[match(amp_batches[,1],rownames(m2)),1],xlab="QC1",ylab="average #UMIs",pch="",log="y")
text(amp_batches$QC1..pooled.cDNA.,m2[match(amp_batches[,1],rownames(m2)),1],labels=amp_batches[,1],cex=.5)
})

# UMIs vs. QC2
try({
plot(amp_batches$QC2..RT.2.,m2[match(amp_batches[,1],rownames(m2)),1],xlab="QC2",ylab="average #UMIs",pch="",log="y")
text(amp_batches$QC2..RT.2.,m2[match(amp_batches[,1],rownames(m2)),1],labels=amp_batches[,1],cex=.5)
})

# UMIs vs. QC3
try({
plot(amp_batches$QC3..library.Ct.,m2[match(amp_batches[,1],rownames(m2)),1],xlab="QC3",ylab="average #UMIs",pch="",log="y")
...
m4=cbind(amp_batch_ID=rownames(m),amp_batches[rownames(m),c("Experiment_ID","Description","QC2..RT.2.")],m[,match(c("reads","spike_yield","gene_umis","gene_umis_neg_control","noise_estimation","avg_noffsets_per_umi","avg_reads_per_umi"),colnames(m))])
