...
while(<DATA>) {
  ...
  for (my $ci=0; $ci< $#UMI_intervals;$ci+=2){
	$UMI_seq=$UMI_seq.substr($seq2,$UMI_intervals[$ci],$UMI_intervals[$ci+1]);
	$UMI_quality=$UMI_quality.substr($quality2,$UMI_intervals[$ci],$UMI_intervals[$ci+1]);
  }
  $UMI_quality=~ s/-/^/g;
  ...
  for (my $oi=0;$oi<=$#oligos;$oi++){
   ...
   my $new_header = $header1."_barcode=".$similar_oligo."-".$pool_barcode_quality."-".$well_barcode_quality."-".$UMI_quality."-".$pool_barcode."-".$well_barcode."-".$UMI_seq;
  $new_header =~ s/ /_/;
