...
if ($#ARGV < 1|$#ARGV>2) {
 die("usage: demultiplex.pl scdb_path amp_batch [downsampling rate]\n");
}
...
my $offsetab_fn = $scdb_path."/output/offset.tab/".$amp_batch_outdir.".txt";
my $singleofftab_fn = $scdb_path."/output/singleton_offset.tab/".$amp_batch_outdir.".txt";
...
my $debug_offsets_fn = $scdb_path."/_debug/".$amp_batch_outdir."/offsets.txt";
...
my $read_stats_fn = $scdb_path."/output/QC/read_stats/".$amp_batch_outdir.".txt";
my $read_stats_amp_batch_fn = $scdb_path."/output/QC/read_stats_amp_batch/".$amp_batch_outdir.".txt";
my $umi_stats_fn = $scdb_path."/output/QC/umi_stats/".$amp_batch_outdir.".txt";
my $umi_nuc_per_pos_fn = $scdb_path."/output/QC/umi_nuc_per_pos/".$amp_batch_outdir.".txt";
my $noffsets_per_umi_fn = $scdb_path."/output/QC/noffsets_per_umi_distrib/".$amp_batch_outdir.".txt";
my $nreads_per_umi_fn = $scdb_path."/output/QC/nreads_per_umi_distrib/".$amp_batch_outdir.".txt";
...
sub filter_errors {
  my($hash_in_ref, $hash_out,$UMI_hash,$cur_gene,$offsets_p1_ref,$offsets_p2_ref,$hamming_thresh_UMI,$hamming_thresh_cell_barcode) = @_;
  my %filt_UMI_hash=();
...
#	}
  ...
  for (my $i=0;$i<=$#all_wellids;$i++){
	...
	my(@UMIs) = sort { $UMI_count{$wellid}->{$a} <=> $UMI_count{$wellid}->{$b} } @all_UMIs;
	for(my($i) = 0; $i <= $#UMIs; $i++) {
	  my($UMI) = $UMIs[$i];
	  ...
	  for(my($j) = $i + 1; $j <=$#UMIs;$j++) {
	    my($dom_UMI) = $UMIs[$j];
		if(hamming($UMI, $dom_UMI,$hamming_thresh_UMI+1) > $hamming_thresh_UMI) {
	  	  next;
		}
		my($off);
		my($indep) = 0;
  ...
  for (my $i=0;$i<=$#all_UMIs;$i++){
	...
	my(@barcodes) = sort { $barcode_count{$UMI}->{$a} <=> $barcode_count{$UMI}->{$b} } @all_barcodes;
	for(my($i) = 0; $i <= $#barcodes; $i++) {
	  ...
	  for(my($j) = $i + 1; $j <= $#barcodes; $j++) {
		...
		my($indep) = 0;
		foreach $off (@{$i_offs}) {
		  if(!exists($barcode_off_map{$UMI}->{"$dom_barcode\t$off"})) {
			$indep = 1;
			last;
		  }
		}
...
#	print $cur_gene."\t".$offset."\t".$stats."\n";
  ...
  foreach my $s (keys %hash_in){
	...
	my $s2=$s."\t".$hash_in{$s}."\t".$filt_UMI_hash{$wellid."_".$UMI}."\t".$filt_barcode_hash{$barcode."_".$UMI}."\t".$offsets_p1_ref->{$cur_gene."\t".$offset}."\t".$offsets_p2_ref->{$cur_gene."\t".$offset}."\t".$offset_stats_hash{$cur_gene."\t".$offset};
...
open(GENE_INTERVALS_FILE, $gene_intervals_fn) || die "ERROR: cannot open file $gene_intervals_fn.\n";
...
my %spike_ins;
open(SPIKE_FILE, $spike_seq_fn) || die "ERROR: cannot open file $spike_seq_fn.\n";
<SPIKE_FILE>;
while(<SPIKE_FILE>) {
  my @l=split("\t",$_);
  $spike_ins{$l[0]}=1;
...
open(AMP_BATCHES_FILE, $amp_batches_fn) || die "ERROR: cannot open file $amp_batches_fn.\n";
...
open(INDEX_FILE, $wells_cells_fn) || die "ERROR: cannot open file $wells_cells_fn.\n";
...
foreach my $fn (@fn) {
  open(DATA, $fn) || die "ERROR: cannot open file $fn.\n";
  print "Reading $fn...";
  my $ii=0;
  while(<DATA>) {
	chomp;
...
open(OUT_DEBUG_OFFSETS, ">$debug_offsets_fn") || die "ERROR: cannot open file $debug_offsets_fn.\n";
print OUT_DEBUG_OFFSETS "gene_name\toffset\twell_id\tbarcode\tUMI\treads\tfiltered_UMI_err\tfiltered_barcode_err\tp_lonely_offset\tp_readpoor_offset\tn_total_lonely_offsets\tn_total_offsets_with_friends\tn_current_lonely_offsets\tn_current_offsets_with_friends\tn_total_offsets_with_single_read\tn_total_offsets_with_multiple_reads\tn_current_offsets_with_single_read\tn_current_offsets_with_multiple_read\n";
...
for (my $j=0; $j<=$#genes;$j++){
  ...
  filter_errors(\%{$hash1{$genes[$j]}},\%{$hash2{$genes[$j]}},\%UMI_hash,$genes[$j],\%offsets_p1,\%offsets_p2,$hamming_thresh_UMI,$hamming_thresh_cell_barcode);
...
open(UMITAB_FILE, ">$umitab_fn") || die "ERROR: cannot open file $umitab_fn.\n";
open(OFFSETAB_FILE, ">$offsetab_fn") || die "ERROR: cannot open file $offsetab_fn.\n";
open(SINGLEOFFTAB_FILE, ">$singleofftab_fn") || die "ERROR: cannot open file $singleofftab_fn.\n";
...
# Iterating over all IVT products of the gene
	my($off,$wellid,$barcode, $UMI,$nreads,$filt1,$filt2,$p1,$p2) = split("\t", $data[$i]);
  ...
  for (my $i=0;$i<=$#well_list;$i++){
	...
	foreach my $UMI (@UMIs){
	  $umi_stats{$wellid."\t".$spike_or_gene."\t".$umi_status{$wellid."\t".$gene."\t".$UMI}}++;
	}
...
open(OUT_READ_STATS_AMP_BATCH, ">$read_stats_amp_batch_fn") || die "ERROR: cannot open file $read_stats_amp_batch_fn.\n";
print OUT_READ_STATS_AMP_BATCH "nreads_with_unknown_cell_barcode\tnreads_bad_quality_well_barcode\tnreads_bad_quality_pool_barcode\tnreads_bad_quality_UMI\tnreads_amp_batch\tnreads_seq_batch\n";

print OUT_READ_STATS_AMP_BATCH "$counter_reads_with_unknown_barcodes\t$counter_bad_quality_pool_barcodes\t$counter_bad_quality_well_barcodes\t$counter_bad_quality_UMI\t$counter_amp_batch_reads\t$counter_seq_batch_reads\n";
...
open(OUT_READ_STATS_PER_CELL, ">$read_stats_fn") || die "ERROR: cannot open file $read_stats_fn.\n";
print OUT_READ_STATS_PER_CELL "well_id\tunmapped\tnon_unique_mapping\tlow_mapq\tmapped_to_nongenic\tspike_UMI_err\tspike_barcode_err\tspike_lonely_offset_err\tspike_readpoor_offset_err\tspike_mapped\tgene_UMI_err\tgene_barcode_err\tgene_lonely_offset_err\tgene_readpoor_offset_err\tgene_mapped\t".$oligos_header."total\n";
...
open(OUT_UMI_STATS_PER_CELL, ">$umi_stats_fn") || die "ERROR: cannot open file $umi_stats_fn.\n";
print OUT_UMI_STATS_PER_CELL "well_id\tgene_UMI_err\tgene_barcode_err\tgene_lonely_offset_err\tgene_readpoor_offset_err\tgene_ok\tspike_UMI_err\tspike_barcode_err\tspike_lonely_offset_err\tspike_readpoor_offset_err\tspike_ok\n";
for (my $j=0; $j<=$#well_list;$j++){
  my $well_id=$well_list[$j];
  print OUT_UMI_STATS_PER_CELL "$well_id\t".(0+$umi_stats{"$well_id\tgene\t-1"})."\t".(0+$umi_stats{"$well_id\tgene\t-2"})."\t".(0+$umi_stats{"$well_id\tgene\t-3"})."\t".(0+$umi_stats{"$well_id\tgene\t-4"})."\t".(0+$umi_stats{"$well_id\tgene\t1"})."\t".(0+$umi_stats{"$well_id\tspike\t-1"})."\t".(0+$umi_stats{"$well_id\tspike\t-2"})."\t".(0+$umi_stats{"$well_id\tspike\t-3"})."\t".(0+$umi_stats{"$well_id\tspike\t-4"})."\t".(0+$umi_stats{"$well_id\tspike\t1"})."\n";
}

close(OUT_UMI_STATS_PER_CELL);

open(OUT_NUC_PER_POS_STATS, ">$umi_nuc_per_pos_fn") || die "ERROR: cannot open file $umi_nuc_per_pos_fn.\n";
...
open(OUT_DEBUG_UMIS, ">$debug_umis_fn") || die "ERROR: annot open file $debug_umis_fn.\n";
print OUT_DEBUG_UMIS "well_id\tgene\tumi\tis_spike\tstatus\tnreads\tn_offsets\n";
for (my $i=0;$i<=$#umis;$i++){
  my $is_spike=0;
  my @arr=split("\t",$umis[$i]);
  ...
  if ($umi_status{$umis[$i]}==1){
	...
	$noffsets_per_umi_hash{$is_spike."\t".$number_of_cells{$well_id}."\t".$noffsets_per_cur_umi}++;
	
	if ($nreads_per_cur_umi>100){
	  $nreads_per_cur_umi=100
	}
	$nreads_per_umi_hash{$is_spike."\t".$number_of_cells{$well_id}."\t".$nreads_per_cur_umi}++;
  }
  print OUT_DEBUG_UMIS $umis[$i]."\t".$is_spike."\t".$umi_status{$umis[$i]}."\t".$reads_per_umi{$umis[$i]}."\t".$offsets_per_umi{$umis[$i]}."\n";
}
close(OUT_DEBUG_UMIS);

open(OFFSETS_PER_UMI, ">$noffsets_per_umi_fn") || die "ERROR: cannot open file $noffsets_per_umi_fn.\n";
print OFFSETS_PER_UMI "noffsets_per_umi\tspikes\tspike_neg_ctrl\tgenes\tgenes_neg_ctrl\n";
for (my $i=0;$i<=20;$i++){
  print OFFSETS_PER_UMI $i."\t".(0+	$noffsets_per_umi_hash{"1\t1\t$i"})."\t".(0+$noffsets_per_umi_hash{"1\t0\t$i"})."\t".(0+$noffsets_per_umi_hash{"0\t1\t$i"})."\t".(0+$noffsets_per_umi_hash{"0\t0\t$i"})."\n";
}
close(OFFSETS_PER_UMI);

open(READS_PER_UMI, ">$nreads_per_umi_fn") || die "ERROR: cannot open file $nreads_per_umi_fn.\n";
print READS_PER_UMI "noffsets_per_umi\tspikes\tspike_neg_ctrl\tgenes\tgenes_neg_ctrl\n";
for (my $i=0;$i<=100;$i++){
 print READS_PER_UMI $i."\t".(0+	$nreads_per_umi_hash{"1\t1\t$i"})."\t".(0+$nreads_per_umi_hash{"1\t0\t$i"})."\t".(0+$nreads_per_umi_hash{"0\t1\t$i"})."\t".(0+$nreads_per_umi_hash{"0\t0\t$i"})."\n";
}
close(READS_PER_UMI);
